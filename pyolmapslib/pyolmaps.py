...
class maps:
    ...
    def addradcircle(self, radcircleradius):
        # radcircleradius = [radcirclecenter, radius, radiuscolor]
    ...
    def draw(self, htmlfile):
        ...
        if len(self.points) > 0 or len(self.roadpoints) > 0:
            ...
            f.write('\t\t});\n')
            f.write('\n')
        if len(self.radcircleradius) > 0:
            print(self.radcircleradius)
    ...
    def drawroad(self, f):
        ...
        f.write('\t\t\tif (count < roadpointarray.length) {\n')
        f.write('\t\t\t\tcolor = roadpointarray[count][2]\n')
        f.write('\t\t\t\twhile (count < roadpointarray.length && roadpointarray[count][2] == color){\n')
        f.write('\t\t\t\t\tif (count != roadpointarray.length-1){\n')
    ...
    def drawradcircle(self, f):
        radcircleradiusstring = 'var radcircleradiusarray = ['
        for i, x in enumerate(self.radcircleradius):
            if i != len(self.radcircleradius) - 1:
                radcircleradiusstring = radcircleradiusstring + '[' + str(x[0]) + ', ' + str(x[1]) + ', ' + str(x[2]) + '], '
            else:
                radcircleradiusstring = radcircleradiusstring + '[' + str(x[0]) + ', ' + str(x[1]) + ', ' + str(x[2]) + ']'
        radcircleradiusstring = radcircleradiusstring + '];'
        f.write('\t\t%s\n' % radcircleradiusstring)
        f.write('\n')
        f.write('\t\tfor (i = 0; i < radcircleradiusarray.length; i++){\n')
        ...
        f.write('\t\t\t\t\tcolor: "rgba(0, 0, 255, 0.2)"\n')
        f.write('\t\t\t\t})\n')
        f.write('\t\t\t})\n')
        f.write('\n')
        f.write('\t\t\tvar radcirclecenter = ol.proj.fromLonLat([radcircleradiusarray[i][1], radcircleradiusarray[i][0]]);\n')
        f.write('\t\t\tvar radcirclelayer = new ol.layer.Vector({\n')
        f.write('\t\t\t\tsource: new ol.source.Vector({\n')
        f.write('\t\t\t\t\tprojection: "EPSG:4326",\n')
        f.write('\t\t\t\t\tfeatures: [new ol.Feature(new ol.geom.Circle(radcirclecenter, radcircleradiusarray[i][2]))]\n')
        f.write('\t\t\t\t}),\n')
        f.write('\t\t\t});\n')
        f.write('\n')
